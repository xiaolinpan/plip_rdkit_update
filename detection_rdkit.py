...
from plip_rdkit.utils import whichresnumber, whichrestype, whichchain, get_atom_coords
...
def filter_contacts(pairings):
    ...
    input:
        pairing: list[atom_namedtuple_A, atom_namedtuple_B],  which is the atom pair of a interaction.
    output:
        return a list, list[atom_namedtuple_A, atom_namedtuple_B], which is filtered by the relu1 and relu2.
    """
    if not config.INTRA:
        return pairings
...
def hydrophobic_interactions(atom_set_a, atom_set_b):
    ...
    input:
        atom_set_a: list[hydrophobic_atom_namedtuple]
        atom_set_b: list[hydrophobic_atom_namedtuple]
    output:
        return interaction pairing, list[atom_namedtuple_hydrophobic]
    """
    data = namedtuple('hydroph_interaction', 'bsatom bsatom_orig_idx ligatom ligatom_orig_idx sidechain '
    ...
    for a, b in itertools.product(atom_set_a, atom_set_b):
        ...
        contact = data(bsatom=a.atom, bsatom_orig_idx=a.orig_idx, ligatom=b.atom, ligatom_orig_idx=b.orig_idx,
                       sidechain=is_sidechain_hc, distance=e, restype=restype, resnr=resnr,
...
def hbonds(acceptors, donor_pairs, protisdon, typ):
    ...
    input:
        acceptors: list[acceptor_namedtuple], which is a acceptor list, every unit in the list is a namedtuple that stores the acceptor info.
        donor_pairs: list[donor_namedtuple], which is a donor list, every unit in the list is a namedtuple that stores the donor info.
        protisdon: boolean, if the protein atom is the donor. 
        typ: string, hbond type (strong or weak)
    output:
        return interaction pairing, list[hbond_namedtuple]
    """
    ...
    for acc, don in itertools.product(acceptors, donor_pairs):
        ...
        if not config.MIN_DIST < dist_ad < config.HBOND_DIST_MAX:
            continue
        vec1, vec2 = vector(don.h_coords, don.d_coords), vector(don.h_coords, acc.coords)
...
def pistacking(rings_bs, rings_lig):
    ...
    input:
        ...
        rings_lig: list[],  
    output:
        return a list[], which contains pi-stacking namedtuple.
    """
    data = namedtuple(
        'pistack',
...
def halogen(acceptor, donor):
    ...
    for acc, don in itertools.product(acceptor, donor):
        ...
        if not config.MIN_DIST < dist < config.HALOGEN_DIST_MAX:
            continue
        vec1, vec2 = vector(acc.o_coords, acc.y_coords), vector(acc.o_coords, don.x_coords)
        vec3, vec4 = vector(don.x_coords, acc.o_coords), vector(don.x_coords, don.c_coords)
...
def water_bridges(bs_hba, lig_hba, bs_hbd, lig_hbd, water):
    ...
    for w in water:
        ...
        for don1 in lig_hbd:
            ...
            d_angle = vecangle(vector(don1.h_coords, don1.d_coords), vector(don1.h_coords, w.oxy_coords))
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST \
                    and d_angle > config.WATER_BRIDGE_THETA_MIN:
                lig_dw.append((don1, w, dist, d_angle))
        for don2 in bs_hbd:
            dist = euclidean3d(don2.d_coords, w.oxy_coords)
            d_angle = vecangle(vector(don2.h_coords, don2.d_coords), vector(don2.h_coords, w.oxy_coords))
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST \
                    and d_angle > config.WATER_BRIDGE_THETA_MIN:
    ...
    for l, p in itertools.product(lig_aw, prot_hw):
        ...
        w_angle = vecangle(vector(acc.coords, wl.oxy_coords), vector(wl.oxy_coords, don.h_coords))
...
def metal_complexation(metals, metal_binding_lig, metal_binding_bs):
    ...
    for cnum, metal in enumerate(pairings_dict):
        ...
        for contact_pair in contact_pairs:
            ...
            vectors_dict[target.atom.GetIdx()].append(vector(get_atom_coords(metal), target.coords))
