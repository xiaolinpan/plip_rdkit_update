...
def whichrestype(atom):
    ...
    return res_info.GetResidueName().strip(" ") if res_info != None else None 

def whichatomname(atom):
    res_info = atom.GetPDBResidueInfo()
    return res_info.GetName().strip(" ") if res_info != None else None 

def whichresnumber(atom):
    res_info = atom.GetPDBResidueInfo()
    return res_info.GetResidueNumber() if res_info != None else None 

def whichchain(atom):
    res_info = atom.GetPDBResidueInfo()
    return res_info.GetChainId() if res_info != None else None 
...
def residue_order(mol):
    ...
    data = namedtuple('residue', 'residue_number residue_name residue_chain residue_atoms')
    residues = []
    for key, value in residue_dict.items():
        atoms = value
        residue_number = int(key.split("_")[0])
        residue_name = list(set([a.GetPDBResidueInfo().GetResidueName() for a in atoms]))
        residue_chain = key.split("_")[2]
        if len(residue_name) != 1:
            raise RuntimeError("get residue iterator error")
        residues.append(data(residue_number=residue_number, residue_name=residue_name[0], residue_chain=residue_chain, residue_atoms=atoms))
    return residues
