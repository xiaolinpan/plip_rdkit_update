...
from plip_rdkit.utils import centroid, tilde_expansion, tmpfile, classify_by_name, get_atom_coords
from plip_rdkit.utils import cluster_doubles, is_lig, normalize_vector, vector, ring_is_planar
from plip_rdkit.utils import extract_pdbid, read_pdb, create_folder_if_not_exists, canonicalize
from plip_rdkit.utils import read, nucleotide_linkage, sort_members_by_importance, is_acceptor, is_donor
from plip_rdkit.utils import whichchain, whichatomname, whichrestype, whichresnumber, euclidean3d, int32_to_negative
...
#it's the base class of ligand and protein class and includes the methods for finding different type atom.
class Mol:
    ...
    def find_rings(self):
        ...
        input: 
            None
        output:
            rings: aromatic_ring_namedtuple, which contains the ring information (atoms orig_atoms atoms_orig_idx normal obj center type).
                   atoms: list[rdkit_atom_obj]
                   orig_atoms: list[atom_index]
                   atoms_orig_idx: list[atom_index], it's same as orig_atoms
                   normal: the normal vector of the ring
                   obj: list[atom_index] 
                   center: the coordinate of the ring center
                   type: the type of ring, 3-membered, 4-membered, 5-membered, 6-membered, ...
        """
    ...
    def find_hal(self):
        ...
        input:
            None
        output:
            a_set: list[hal_acceptor_namedtuple]
                o: rdkit atom object 
                o_orig_idx: rdkit atom index
                y: rdkit atom object
                y_orig_idx: rdkit atom index
                o_coords: coordinates
                y_coords: coordinates
        """
        data = namedtuple('hal_acceptor', 'o o_orig_idx y y_orig_idx o_coords y_coords')
        a_set = []
        # All oxygens, nitrogen, sulfurs with neighboring carbon, phosphor, nitrogen or sulfur
        for a in [at for at in self.mol.GetAtoms() if at.GetAtomicNum() in [8, 7, 16]]:
...
class Ligand(Mol):
    def __init__(self, mol, mol_water):
        ...
        input:
            mol: rdkit mol object
            mol_water: rdkit mol object of water
        output:
            None
        """
        ...
        self.max_dist_to_center = max((euclidean3d(self.centroid, get_coord(self.mol_conf, a.GetIdx())) for a in mol.GetAtoms()))
        ...
        if mol_water is not None:
            ...
            for hoh in self.mol_water.GetAtoms():
                ...
                if hoh.GetAtomicNum() == 8:
                    oxy = hoh
                if oxy is not None:
                    if euclidean3d(self.centroid, get_atom_coords(oxy)) < self.max_dist_to_center + config.BS_DIST:
                        oxy_orig_idx = oxy.GetIdx()
                        oxy_coords = get_atom_coords(oxy)
                        self.water.append(data(oxy=oxy, oxy_orig_idx=oxy_orig_idx, oxy_coords=oxy_coords))
    ...
    def hydrophobic_atoms(self):
        ...
        for atom in atm:
            ...
            orig_atom = orig_idx
            atom_set.append(data(atom=atom, orig_atom=orig_atom, orig_idx=orig_idx, coords=get_atom_coords(atom)))
        return atom_set
    ...
    def find_hba(self):
        ...
        for atom_idx, atom in zip(atom_idx_set, atm):
            if atom.GetAtomicNum() not in [9, 17, 35, 53]:  # Exclude halogen atoms
                ...
                a_set.append(data(a=atom, a_orig_atom=a_orig_atom, a_orig_idx=a_orig_idx, type='regular', coords=coords))
            ...
            if not in_ring:
                for adj_atom in [a for a in donor_atom.GetNeighbors() if a.GetAtomicNum() == 1]:
                    ...
                    donor_pairs.append(data(d=donor_atom, d_orig_atom=d_orig_atom, d_orig_idx=d_orig_idx,
                                            h=adj_atom, type='regular', d_coords=d_coords, h_coords=h_coords))
        ...
        for carbon in self.hydroph_atoms:
            for adj_atom in [a for a in carbon.atom.GetNeighbors() if a.GetAtomicNum() == 1]:
                ...
                donor_pairs.append(data(d=carbon, d_orig_atom=d_orig_atom,
                                        ...
                                        d_coords=d_coords, h_coords=h_coords))
    ...
    def find_metal_binding(self):
        ...
        """
        a_set = []
        data = namedtuple('metal_binding', 'atom orig_atom atom_orig_idx type fgroup restype resnr reschain location coords')
        for oxygen in self.water:
            a_set.append(data(atom=oxygen.oxy, atom_orig_idx=oxygen.oxy_orig_idx, 
                              ...
                              reschain="W", coords=oxygen.oxy_coords,
        ...
        for a in self.mol.GetAtoms():
            ...
            if a.GetAtomicNum() == 8:  # Oxygen
                if n_atoms_atomicnum.count('1') == 1 and len(n_atoms_atomicnum) == 2:  # Oxygen in alcohol (R-[O]-H)
                    a_set.append(data(atom=a, atom_orig_idx=a_orig_idx, type='O', coords=get_atom_coords(a), 
                ...
                if n_atoms_atomicnum.count(8) == 2 and n_atoms_atomicnum.count(6) == 1:  # It's a carboxylate group
                    for neighbor in [n for n in n_atoms if n.GetAtomicNum() == 8]:
                        ...
                        a_set.append(data(atom=neighbor, atom_orig_idx=neighbor_orig_idx, type='O',
                                          ...
                                          location='ligand', orig_atom=a_orig_idx, coords=get_atom_coords(neighbor)))
            ...
            if a.GetAtomicNum() == 7:  # It's a nitrogen atom
                if n_atoms_atomicnum.count(6) == 2:  # It's imidazole/pyrrole or similar
                    a_set.append(data(atom=a, atom_orig_idx=a_orig_idx, type='N', coords=get_atom_coords(a), 
            ...
            if a.GetAtomicNum() == 16:  # It's a sulfur atom
                if True in [n.GetIsAromatic() for n in n_atoms] and not a.GetIsAromatic():  # Thiolate
                    a_set.append(data(atom=a, atom_orig_idx=a_orig_idx, type='S', coords=get_atom_coords(a),
...
class Protein(Mol):
    def __init__(self, mol):
        ...
        for a in [a for a in self.all_atoms if a.GetSymbol().upper() in config.METAL_IONS]:
            m_orig_idx = a.GetIdx()
            orig_m = m_orig_idx
            self.metals.append(data(m=a, m_orig_idx=m_orig_idx, orig_m=orig_m, m_coords=get_atom_coords(a)))
    ...
    def find_hba(self):
        ...
        for atom in self.all_atoms:
            if is_acceptor(atom):
                ...
                a_orig_atom = atom
                a_set.append(data(a=atom, a_orig_atom=a_orig_atom, a_orig_idx=a_orig_idx, 
                                  type='regular', coords=get_atom_coords(atom)))
        a_set = sorted(a_set, key=lambda x: x.a_orig_idx)
        return a_set
    ...
    def find_hbd(self):
        ...
        for carbon in self.hydroph_atoms:
            for adj_atom in [a for a in carbon.atom.GetNeighbors() if a.GetAtomicNum() == 1]:
                ...
                d_coords = get_atom_coords(carbon.atom)
                h_coords = get_atom_coords(adj_atom)
                donor_pairs.append(data(d=carbon, d_orig_atom=d_orig_atom, d_coords=d_coords, 
                                        h_coords=h_coords, d_orig_idx=d_orig_idx, h=adj_atom, 
                                        type='weak'))
        donor_pairs = sorted(donor_pairs, key=lambda x: (x.d_orig_idx, x.h.GetIdx()))
        return donor_pairs
            ...
            if res.residue_name in ("ARG", "HIS", "LYS"):
                ...
                if not len(a_contributing) == 0:
                    a_set.append(data(atoms=a_contributing,
                                      ...
                                      type='positive',
                                      center=centroid([get_atom_coords(ac) for ac in a_contributing]),
    ...
    def find_metal_binding(self):
        ...
        data = namedtuple('metal_binding', 'atom atom_orig_idx type restype resnr reschain location coords')
        a_set = []
        for res in self.residues:
                ...
                for a in res.residue_atoms:
                    if a.GetSymbol() == "O" and a.GetPDBResidueInfo().GetName().strip(" ") != "O":
                        ...
                        a_set.append(data(atom=a, atom_orig_idx=atom_orig_idx, type='O', restype=restype,
                                          resnr=resnr, reschain=reschain, coords=get_atom_coords(a),
            ...
            for a in res.residue_atoms:  # All main chain oxygens
                if a.GetSymbol() == "O" and a.GetPDBResidueInfo().GetName().strip(" ") == "O":
                    ...
                    a_set.append(data(atom=a, atom_orig_idx=atom_orig_idx, type='O', restype=res.residue_name,
                                      resnr=res.residue_number, reschain=reschain, coords=get_atom_coords(a),
...
class PLInteraction:
    ...
    def __init__(self, lig_obj, bs_obj, pdbid):
        ...
        self.all_water_bridges = water_bridges(self.protein.get_hba(), self.ligand.get_hba(),
                                           self.protein.get_hbd(), self.ligand.get_hbd(),
                                           self.ligand.water)

        self.water_bridges = self.refine_water_bridges(self.all_water_bridges, self.hbonds_ldon, self.hbonds_pdon)
